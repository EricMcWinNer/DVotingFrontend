...
class FingerprintHandler extends PureComponent {
      ...
      if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
        ...
        if (fpobject.ErrorCode === 0) {
          ...
          this.setState(
            ...
            () => {
              this.props.onSuccessfulScan(this.state.template);
            }
          );
        } else {
          this.setState(
          ...
          } else if (fpobject.ErrorCode === 53) {
            this.displayAlert(
              "Sensor line dropped!",
              "The fingerprint reader wasn't properly recognized by your system. Unplug it and try again. If the problem persists, restart your computer and try again."
            );
          } else if (fpobject.ErrorCode === 61) {
            this.displayAlert(
              "Unsupported Device",
              "The fingerprint you are using is not supported. Ensure you are using the recommended fingerprint device"
            );
          } else if (
            fpobject.ErrorCode === 1 ||
            fpobject.ErrorCode === 2 ||
            fpobject.ErrorCode === 5 ||
            fpobject.ErrorCode === 6 ||
            fpobject.ErrorCode === 7 ||
            fpobject.ErrorCode === 51
          ) {
            this.displayAlert(
              "Driver files error!",
          ...
          // DO SOMETHING, PROBABLY STONE THEM SWEETALERT
        }
        //eslint-disable-next-line
  ...
  render() {
    const { label, className, defaultTemplate, icon, ...props } = this.props;
    return (
      <div
        {...props}
      ...
      >
        ...
        <div
          ...
          }${this.state.template === null ? "" : " captured"}
